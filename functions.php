...
function york_scripts() {
	...
	// Scripts.
	wp_enqueue_script( 'york-global', get_theme_file_uri( '/assets/js/global.js' ), array( 'jquery', 'masonry', 'imagesloaded' ), '1.1.2', true );
	wp_enqueue_script( 'york-skip-link-focus-fix', get_theme_file_uri( '/assets/js/skip-link-focus-fix.js' ), array(), '1.1.2', true );
...
function yearsExperience_func( $atts ) {
    $year = $atts['year'];
    $exp = date("Y") - $year;
    if($exp == 0){
        return "<1";
